...
from core.sandbox_eval import build_diagnostic_packet, finite_metric_mapping, held_out_metric_failure
...
def main():
    ...
    assert held_out_metric_failure(
        {"mse": 0.1, "rmse": np.sqrt(0.1)},
        {"mse": 0.2, "rmse": np.sqrt(0.2)},
    ) is None
    metric_failure = held_out_metric_failure(
        {"mse": 0.1, "rmse": np.sqrt(0.1)},
        {"mse": np.nan, "rmse": np.inf},
    )
    assert metric_failure is not None
    assert metric_failure["category"] == "nonfinite_held_out_metrics"
    assert "test.mse" in metric_failure["detail"]
    assert "test.rmse" in metric_failure["detail"]
    assert finite_metric_mapping({"mse": np.nan, "rmse": np.inf}) == {"mse": None, "rmse": None}

    nonfinite_packet = build_diagnostic_packet(
        res={
            "median_distance": 0.1,
            "min_distance": 0.05,
            "accepted_params": np.array([[0.3], [0.31]]),
        },
        metadata=[{"name": "alpha", "range": (0.0, 1.0)}],
        status="failed",
        train_metrics={"mse": 0.1, "rmse": np.sqrt(0.1)},
        test_metrics={"mse": None, "rmse": None},
        extra_failure_modes=[metric_failure],
    )
    assert nonfinite_packet["status"] == "failed"
    assert "nonfinite_held_out_metrics" in {
        mode["category"] for mode in nonfinite_packet["failure_modes"]
    }
