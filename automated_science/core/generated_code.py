...
def validate_generated_math_code(code: str) -> None:
    ...
    for node in tree.body:
        ...
        if isinstance(node, ast.FunctionDef):
            ...
            if node.args.defaults or node.args.kw_defaults:
                raise GeneratedCodeError("dynamics(t, y, args) must not define default arguments")
            positional_args = [
                *node.args.posonlyargs,
                *node.args.args,
            ]
            arg_names = [arg.arg for arg in positional_args]
            if arg_names != ["t", "y", "args"]:
                raise GeneratedCodeError("dynamics must have the exact signature dynamics(t, y, args)")
            if node.args.vararg or node.args.kwarg or node.args.kwonlyargs:
                raise GeneratedCodeError("dynamics(t, y, args) must not define variadic or keyword-only arguments")
            dynamics_node = node
            continue
    ...
    if dynamics_node is None:
        raise GeneratedCodeError("missing dynamics(t, y, args)")
