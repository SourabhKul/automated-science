...
def evaluate_subject_holdout_candidate(raw_code: str, config: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    diagnostics = build_diagnostic_packet(
        ...
        metadata=metadata,
        status="success",
        train_metrics=train_subject_metrics["aggregate"],
        test_metrics=test_subject_metrics["aggregate"],
    )
...
def main() -> int:
    ...
    try:
        ...
        if config.get("evaluation_mode") == "subject_holdout":
            results.update(evaluate_subject_holdout_candidate(raw_code, config, args))
...
"""
        ...
        if args.held_out and res["median_distance"] < float("inf"):
        ...
        results["status"] = "success" if res["median_distance"] < float("inf") else "failed"
