...
\end{equation}
where $\mathbf{y}$ is the state vector, $t$ is time, and $\theta$ is the parameter set. The function $f$ is generated as raw JAX/Python code by the LLM and solved using the Diffrax library via the Tsitouras 5/4 (Tsit5) adaptive solver. By generating raw code rather than manipulating abstract syntax trees, the LLM can effortlessly inject arbitrary JAX primitives, such as `jnp.where` for discontinuous boundary conditions or `jnp.sin` for external periodic forcing.
