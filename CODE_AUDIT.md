...
4. **Baseline script now supports integrated trajectory diagnostics, but historical $E^3$ runs are not yet matched to that protocol.**
   - Evidence: `scripts/legacy/run_baselines.py` rolls PySINDy and PySR-discovered dynamics forward to saved train/test trajectory metrics, and keeps PySR derivative errors as diagnostics. New `scripts/run_domain.py --held-out` runs fit on the first 80% of the trajectory and select accepted hypotheses by held-out test MSE.
...
5. **Domain orchestrators are heavily duplicated.**
   - Evidence: 50 qwen36/gemma4 domain orchestrators were consolidated into the config-driven runner; the remaining compatibility runners are retained in the local cleanup backup rather than the public tree.
   - Risk: historical runners are not part of the maintained extension surface.
   - Fix path: add new behavior to `core/domain_configs.py` and `scripts/run_domain.py`; use the local archive only for historical replay.
...
- `python -m tests.test_generated_code`
- `python -m tests.test_sbi_engine`
- `python -m tests.test_sandbox_eval`
- `python -m tests.test_run_domain_config`
- `python -m tests.test_baselines`
- `python -m tests.test_stress`
- the historical results exporter, preserved in the local cleanup backup
- `python scripts/audit_results.py`
- `python scripts/legacy/run_baselines.py --domains ecology --pysr-iterations 1 --output artifacts/baselines/baseline_smoke.json`
