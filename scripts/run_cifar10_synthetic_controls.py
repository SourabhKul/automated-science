#!/usr/bin/env python3
"""Output-isolated artificial recovery and specificity checks for Phase 58."""
...
OUTPUT_DIR = Path("artifacts/evaluations/phase58_cifar10_synthetic_controls_20260731")
SEED = 2058001
...
class LinearArtificialClassifier:
    ...
    def score(self, images: np.ndarray, labels: np.ndarray) -> float:
        ...
        return float(f1_score(labels, predictions, labels=LABELS, average="macro"))
...
def artificial_images(offset: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    for label in LABELS:
        ...
        for repeat in range(8):
            rng = np.random.default_rng(SEED + offset * 10_000 + label * 100 + repeat)
...
def block_ablation_drops(model: LinearArtificialClassifier, images: np.ndarray, labels: np.ndarray, score: float) -> list[float]:
    drops: list[float] = []
...
def run(output_dir: Path = OUTPUT_DIR) -> dict[str, object]:
    ...
    ablation_drops = block_ablation_drops(model, selection_images, selection_labels, planted_score)

    candidates = [CifarImage(values) for values in selection_images[:8]]
    states: list[float] = []
    ...
    controls = {
        ...
        "batch_rejected": rejects_payload({"values": values, "batch": "data_batch_1"}),
        "file_rejected": rejects_payload({"values": values, "file": "test_batch"}),
    ...
    passed = (
        ...
        and max(ablation_drops) >= 0.05
        and all(bool(value) for key, value in controls.items() if key.endswith("rejected") or key in {"zero_reset", "reorder_invariant", "duplicate_isolation", "finite_probabilities"})
    )
    result = {
        "phase": 58,
