#!/usr/bin/env python3
"""Output-isolated artificial recovery and specificity checks for Phase 59."""
...
from core.real_data.svhn import NORMALIZED_LABELS, SvhnImage, image_from_payload, normalized_label, predict_independent

OUTPUT_DIR = Path("artifacts/evaluations/phase59_svhn_synthetic_controls_20260801")
SEED = 2059001
BLOCK = 8
...
class LinearArtificialClassifier:
    ...
    def score(self, images: np.ndarray, labels: np.ndarray) -> float:
        ...
        return float(f1_score(labels, predictions, labels=NORMALIZED_LABELS, average="macro"))


def artificial_images(offset: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    images: list[np.ndarray] = []
    normalized_labels: list[int] = []
    raw_labels: list[int] = []
    for normalized in NORMALIZED_LABELS:
        raw_label = 10 if normalized == 0 else normalized
        block_row, block_column = divmod(normalized, 4)
        for repeat in range(8):
            rng = np.random.default_rng(SEED + offset * 10_000 + normalized * 100 + repeat)
            image = rng.normal(12.0, 2.0, (3, 32, 32))
            image[:, block_row * BLOCK : (block_row + 1) * BLOCK, block_column * BLOCK : (block_column + 1) * BLOCK] += 190.0
            images.append(np.clip(image, 0.0, 255.0))
            normalized_labels.append(normalized)
            raw_labels.append(raw_label)
    return np.asarray(images), np.asarray(normalized_labels), np.asarray(raw_labels)
...
def run(output_dir: Path = OUTPUT_DIR) -> dict[str, object]:
    train_images, train_labels, train_raw_labels = artificial_images(0)
    selection_images, selection_labels, selection_raw_labels = artificial_images(1)
    if not np.array_equal(np.asarray([normalized_label(int(label)) for label in train_raw_labels]), train_labels):
        raise ValueError("artificial raw-label mapping changed")
    ...
    candidates = [SvhnImage(values) for values in selection_images[:8]]
    states: list[float] = []
    ...
    controls = {
        "raw_label_mapping": normalized_label(10) == 0 and normalized_label(1) == 1 and np.array_equal(np.asarray([normalized_label(int(label)) for label in selection_raw_labels]), selection_labels),
        ...
        "target_rejected": rejects_payload({"values": values, "label": 10}),
        "file_rejected": rejects_payload({"values": values, "file": "test_32x32.mat"}),
    ...
    passed = (
        ...
        and max(drops) >= 0.05
        and all(bool(value) for key, value in controls.items() if key.endswith("rejected") or key in {"raw_label_mapping", "zero_reset", "reorder_invariant", "duplicate_group_isolation", "finite_probabilities"})
    )
    result = {
        "phase": 59,
