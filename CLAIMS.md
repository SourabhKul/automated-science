...
| The repo contains preserved 25-domain, 10-run Qwen result artifacts. | Supported | The local `artifacts/runs/qwen36_27b/` tree contains the primary run history; generated run artifacts are intentionally excluded from the public checkout. | Keep, but treat the local artifact tree as the evidence source. |
| Results table is generated from run-history CSV files. | Partially supported | `paper/results_table.tex` preserves the historical table; the former exporter and run-history tree are archived locally rather than published in the cleaned checkout. | Keep the table as historical evidence; rerun the exporter before regenerating it. |
...
| The paper's baseline comparison is fully apples-to-apples. | Partially supported | `scripts/legacy/run_baselines.py` now saves integrated trajectory metrics for PySINDy/PySR, and new `scripts/run_domain.py --held-out` runs select by held-out test MSE. Historical $E^3$ artifacts were not generated under that protocol. | Rerun selected $E^3$ domains with matched train/test splits and compute budgets before making final comparison claims. |
...
| Generated-code evaluation is fully sandboxed. | Partially supported | Centralized domain runs evaluate candidates in a subprocess via `core/sandbox_eval.py` after AST validation, but this is not an OS security sandbox. Historical compatibility runners are archived outside the public tree. | Treat distributed execution as future work; add real resource/container sandboxing before deployment. |
...
4. Mark baseline comparison as preliminary until selected $E^3$ domains are rerun with the same held-out trajectory protocol used by `scripts/legacy/run_baselines.py`.
